...
void TelaCriarCadastro(){
	...
	if((usuarioTemp = (USUARIO *) malloc(sizeof(USUARIO))) == NULL) exit(0);

	move(((LINES-16)/2)+3, ((COLS-40)/2)+9);
	scanw("%s", &temp);
	if((usuarioTemp->nomeUsuario = (char *) malloc(strlen(temp) * sizeof(char))) == NULL) exit(0);
	strcpy(usuarioTemp->nomeUsuario, temp);
